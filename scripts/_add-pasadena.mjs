...
let c=await geo('19 rue des Écouffes 75004 Paris') || {lat:48.8567,lng:2.3585}
console.log('Pasa Dena:', c)
...
for(const a of articles) if(a.slug==='le-tour-des-fripes-ideal-dans-le-marais'){
  const m=a.mapMarkers||[]; const i=m.findIndex(x=>x.name==='Pasa Dena'); const p={name:'Pasa Dena',kind:'cafe',lat:c.lat,lng:c.lng}
