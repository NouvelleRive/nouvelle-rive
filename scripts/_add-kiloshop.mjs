...
let c=await geo('Kilo Shop 69 rue de la Verrerie Paris') || await geo('69 rue de la Verrerie 75004 Paris') || {lat:48.858,lng:2.3524}
console.log('Kilo Shop:', c)
...
for(const a of articles) if(a.slug==='le-tour-des-fripes-ideal-dans-le-marais'){
  const m=a.mapMarkers||[]; const i=m.findIndex(x=>x.name==='Kilo Shop'); const p={name:'Kilo Shop',kind:'fripe',lat:c.lat,lng:c.lng}
