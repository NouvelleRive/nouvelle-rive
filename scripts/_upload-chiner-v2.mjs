...
const path='journal/chiner-paris-2.jpg'
const buf=readFileSync('/private/tmp/claude-501/-Users-salomekassabi-Desktop-nouvelle-rive/574b8f36-fb06-47a8-8294-2f5892431202/scratchpad/chiner_final.jpg')
...
const url=`${cdn}/${path}`; console.log('✓ uploadé (nouvelle URL):',url)
...
for(const a of articles) if(a.slug==='chiner-vintage-marais'){ a.cover=url }
await ref.set({ articles })
try{ await getStorage().bucket().file('_cache/journal.json.gz').delete() }catch{}
console.log('✓ cover màj')
process.exit(0)
