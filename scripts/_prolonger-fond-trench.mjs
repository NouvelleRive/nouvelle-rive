// Prolonge le fond studio jusqu'aux bords du carré sur les photos des trench.
...
// --- Iconiques trench ---
const icoSnap = await db.collection('iconiques').where('slug', '==', 'trench-burberry').get()
for (const doc of icoSnap.docs) {
  const images = doc.data().images || []
  console.log(`iconique trench-burberry — ${images.length} images`)
...
console.log(`${trenches.length} produits trench`)
