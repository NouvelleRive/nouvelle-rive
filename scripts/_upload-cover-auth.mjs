...
const path='journal/authentification-chanel.png'
const buf=readFileSync('/Users/salomekassabi/Downloads/IMG_0365.PNG')
...
if(!res.ok){ console.log('❌ upload Bunny',res.status,await res.text()); process.exit(1) }
const url=`${cdn}/${path}`
...
for(const a of articles) if(a.slug==='reconnaitre-vrai-sac-luxe-vintage'){ a.cover=url; console.log('✓ couverture màj (article 19 août)') }
