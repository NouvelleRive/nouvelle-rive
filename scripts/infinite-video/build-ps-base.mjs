...
const W = 1080, H = 1920, PERIOD = 60, BLUE = '#22209C', NOM = 'PERSONAL SELLER'
...
const OUT = join(OUTDIR, 'infinite-personal-seller-2rangs.mp4')

let imgs = JSON.parse(readFileSync(join(S, 'ps-order.json'), 'utf8')).filter(Boolean)
console.log(`→ ${imgs.length} tuiles (Chanel + YSL prioritaires)`)
...
const file = join(OUTDIR, '.tmp-ps.html')
