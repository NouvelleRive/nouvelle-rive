...
const W = 1080, H = 1920, PERIOD = 34, BLUE = '#22209C', NOM = "TÊTE D'ORANGE"
...
const OUT = join(OUTDIR, 'infinite-tete-dorange-2rangs.mp4')

let imgs = JSON.parse(readFileSync(join(S, 'tdo-order.json'), 'utf8')).filter(Boolean)  // 19 SKU dans l'ordre imposé
...
const file = join(OUTDIR, '.tmp-tdo.html')
