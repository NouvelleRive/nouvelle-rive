...
const W = 1080, H = 1920, PERIOD = 34, BLUE = '#22209C', NOM = 'INÈS PINEAU'
...
const OUT = join(OUTDIR, 'infinite-ines-pineau-2rangs.mp4')
const DESK = join(homedir(), 'Desktop')

const prods = JSON.parse(readFileSync(join(S, 'cr-ines-fresh.json'), 'utf8')).produits.slice(0, 20)
let imgs = prods.map(p => p.imageUrl).filter(Boolean)
// utiliser LES PHOTOS DU BUREAU (comme les autres) — on remplace juste l'URL par le fichier, trim identique
const byId = (id, file) => { const i = prods.findIndex(p => p.id === id); if (i >= 0 && existsSync(join(DESK, file))) imgs[i] = join(DESK, file) }
byId('6ITuABKnsJrX7SDdw43f', 'IP157.png')
console.log(`→ ${imgs.length} pièces (photos Bureau, trim normal comme les autres)`)

// crop auto du fond -> tailles/espacement réguliers (identique pour toutes)
...
const file = join(OUTDIR, '.tmp-ines.html')
