...
const W = 1080, H = 1920, PERIOD = 34, BLUE = '#22209C', NOM = 'CASTING ARCHIVES'
...
const OUT = join(OUTDIR, 'infinite-casting-archives-2rangs.mp4')

let imgs = JSON.parse(readFileSync(join(S, 'cas-order.json'), 'utf8')).map(r => r.img).filter(Boolean)  // 15 SKU dans l'ordre imposé
console.log(`→ ${imgs.length} pièces`)
imgs = await trimToLocal(imgs)   // crop auto du fond
...
const file = join(OUTDIR, '.tmp-cas.html')
