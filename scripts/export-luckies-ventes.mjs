// Export les ventes des produits Luckies (NAN + LUC) pour avril + mai 2026
...
const luckies = all.docs.map(d => ({ id: d.id, ...d.data() })).filter(v => {
  const trig = (v.trigramme || '').toUpperCase()
  const sku = (v.sku || '').toUpperCase()
  // Tout produit Luckies : trigramme NAN/LUC ou sku commençant par NAN/NG/LUC
  const isLuckies = trig === 'NAN' || trig === 'LUC' || /^(NAN|NG|LUC)/i.test(sku)
  if (!isLuckies) return false
...
console.log(`${luckies.length} ventes Luckies entre ${start.toISOString().slice(0,10)} et ${end.toISOString().slice(0,10)}\n`)

const rows = ['Date,SKU,Nom,Prix vente,Prix initial,Source']
let totalCA = 0
for (const v of luckies) {
