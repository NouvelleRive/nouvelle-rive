  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (let p = 0; p < quantiteLot; p++) {
      ...
      const payload: Record<string, unknown> = {
        ...
        etat: it.etat || '',
        ...(it.modele ? { modele: String(it.modele).trim() } : {}),
        ...(it.motif ? { motif: String(it.motif).trim() } : {}),
        ...(it.sleeveLength ? { sleeveLength: String(it.sleeveLength).trim() } : {}),
        ...(it.collarType ? { collarType: String(it.collarType).trim() } : {}),
        ...(it.garmentLength ? { garmentLength: String(it.garmentLength).trim() } : {}),
        ...(it.closureType ? { closureType: String(it.closureType).trim() } : {}),
        sku,
