...
type Produit = { id: string; chineur?: string; chineurUid?: string; vendu?: boolean; createdAt?: Timestamp }
...
export default function VendeuseCalendrierPage() {
  ...
  const caParVendeuse = useMemo(() => {
    ...
    ventesAll.forEach(p => {
      ...
      const addTo = (id: string, fraction: number) => {
        ...
        map.set(id, { ...cur, ca: cur.ca + montant * fraction, ventes: cur.ventes + fraction, discountCount: cur.discountCount + (hasDiscount ? fraction : 0), discountTotal: cur.discountTotal + discount * fraction })
      }
    ...
    ca1220.forEach((ca, dateStr) => {
      if (ca < 1000) return
      const vid = planningSlots[`${dateStr}_12-20`]
      if (!vid) return
      const cur = map.get(vid)
      if (cur) map.set(vid, { ...cur, bonus: cur.bonus + ca * 0.01 })
    })
    ca1117.forEach((ca, dateStr) => {
      if (ca < 1000) return
      const vid = planningSlots[`${dateStr}_11-17`]
      if (!vid) return
      const cur = map.get(vid)
      if (cur) map.set(vid, { ...cur, bonus: cur.bonus + ca * 0.01 })
    })
  ...
  const restockStats = useMemo(() => {
    return [...deposantes].map(d => {
      ...
      const nbProduits = mine.length
      const dernierProduit = mine.sort((a, b) => {
      ...
      const joursDepuis = dernierRestock
        ...
        : null
      return { d, nbProduits, joursDepuis, dernierRestock }
        ...
        {activeVendeuses.length > 0 && (
          <div className="mt-6 lg:mt-0">
            <div className="bg-white rounded-xl border p-4 sticky top-20">
              <h3 className="text-sm font-bold mb-3">Récap — {monthLabel}</h3>
          ...
          <div className="bg-white rounded-xl border p-4 sticky top-20">
            ...
            <div className="space-y-1">
              {restockStats.map(({ d, nbProduits, joursDepuis, dernierRestock }) => (
                <div
                  key={d.id}
                  className={`flex items-center justify-between py-1 text-xs ${nbProduits < 30 || joursDepuis === null || joursDepuis > 30 ? 'text-red-500' : 'text-gray-700'}`}
                >
