...
export function getMonthEvents(month: number, year: number): RetailEvent[] {
  ...
  pfw.forEach(([py,ms,ds,me,de,label]) => addRange(py,ms,ds,me,de,label,'rgba(0,0,0,0.06)'))
  ...
  soldes.forEach(([sy,ms,ds,me,de]) => addRange(sy,ms,ds,me,de,'Soldes','rgba(220,38,38,0.08)'))
  ...
  vac.forEach(([vy,ms,ds,me,de,label]) => {
    if (vy === y) { if (m >= ms) addRange(y,ms,ds,me,de,label,'rgba(107,114,128,0.08)') }
    if (vy === y - 1 && me === 0) { if (m === 0) add(1, de, label, 'rgba(107,114,128,0.08)') }
  })

  // === US Spring Break (approx mi-mars - mi-avril) ===
  if (m === 2) add(15, daysInMonth, 'US Spring Break', 'rgba(245,158,11,0.06)')
  if (m === 3) add(1, 15, 'US Spring Break', 'rgba(245,158,11,0.06)')

  // === US Summer (juin - aout) ===
  if (m === 5) add(10, daysInMonth, 'US Summer', 'rgba(245,158,11,0.06)')
  if (m === 6) add(1, daysInMonth, 'US Summer', 'rgba(245,158,11,0.06)')
  if (m === 7) add(1, 25, 'US Summer', 'rgba(245,158,11,0.06)')

  // === US Xmas Break ===
  if (m === 11) add(20, daysInMonth, 'US Xmas', 'rgba(245,158,11,0.06)')
  if (m === 0) add(1, 3, 'US Xmas', 'rgba(245,158,11,0.06)')

  // === US feries (1 jour) ===
  const memorialDay: Record<number,number> = {2024:27,2025:26,2026:25}
  if (m===4 && memorialDay[y]) add(memorialDay[y],memorialDay[y],'US Memorial Day','rgba(245,158,11,0.12)')
  const laborDay: Record<number,number> = {2024:2,2025:1,2026:7}
  if (m===8 && laborDay[y]) add(laborDay[y],laborDay[y],'US Labor Day','rgba(245,158,11,0.12)')
  if (m===6) add(4,4,'US 4th July','rgba(245,158,11,0.12)')
  const thanksgiving: Record<number,number> = {2024:28,2025:27,2026:26}
  if (m===10 && thanksgiving[y]) add(thanksgiving[y],thanksgiving[y],'US Thanksgiving','rgba(245,158,11,0.12)')

  // === Black Friday - Cyber Monday ===
  if (m===10 && thanksgiving[y]) add(thanksgiving[y]+1,thanksgiving[y]+4,'BF - CM','rgba(17,17,17,0.1)')

  // === Golden Week Chine ===
  if (m===9) add(1,7,'CN Golden Week','rgba(239,68,68,0.08)')

  // === Jours feries FR (1 jour) ===
  if (m===0) add(1,1,'Nouvel An','rgba(37,99,235,0.12)')
  if (m===4) { add(1,1,'1er Mai','rgba(37,99,235,0.12)'); add(8,8,'8 Mai','rgba(37,99,235,0.12)') }
  if (m===6) add(14,14,'14 Juil.','rgba(37,99,235,0.12)')
  if (m===7) add(15,15,'15 Aout','rgba(37,99,235,0.12)')
  if (m===10) { add(1,1,'Toussaint','rgba(37,99,235,0.12)'); add(11,11,'11 Nov','rgba(37,99,235,0.12)') }
  const paques: Record<number,[number,number]> = {2024:[2,31],2025:[3,20],2026:[3,5]}
  if (paques[y] && m===paques[y][0]) add(paques[y][1],paques[y][1],'Paques','rgba(37,99,235,0.12)')
  const ascension: Record<number,[number,number]> = {2024:[4,9],2025:[4,29],2026:[4,14]}
  if (ascension[y] && m===ascension[y][0]) add(ascension[y][1],ascension[y][1],'Ascension','rgba(37,99,235,0.12)')

  // === Fetes commerciales (1 jour) ===
  if (m===1) add(14,14,'St Valentin','rgba(225,29,72,0.12)')
  if (m===11) add(25,25,'Noel','rgba(22,163,74,0.12)')
  const feteMeres: Record<number,[number,number]> = {2024:[4,26],2025:[4,25],2026:[4,31]}
  if (feteMeres[y] && m===feteMeres[y][0]) add(feteMeres[y][1],feteMeres[y][1],'Fete meres','rgba(217,70,239,0.12)')
